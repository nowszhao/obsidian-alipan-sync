...
import { blobKV, syncRecordKV, traverseCacheKV } from '~/storage/kv'
...
export default class CacheClearModal extends Modal {
	...
	onOpen() {
		...
		// Remote Traversal Cache Option
		new Setting(optionsContainer)
			.setName(i18n.t('settings.cache.clearModal.traverseRemoteCache.name'))
			.setDesc(i18n.t('settings.cache.clearModal.traverseRemoteCache.desc'))
			.addToggle((toggle) => {
				toggle.setValue(this.options.traverseCacheEnabled).onChange((value) => {
					this.options.traverseCacheEnabled = value
				})
			})
	...
	static async clearSelectedCaches(options: CacheClearOptions) {
		const { syncRecordEnabled, blobEnabled, traverseCacheEnabled } = options
		const cleared = []
		...
		try {
			...
			if (traverseCacheEnabled) {
				await traverseCacheKV.clear()
				cleared.push(i18n.t('settings.cache.clearModal.traverseRemoteCache.name'))
			}
