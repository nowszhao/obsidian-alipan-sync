...
export default class TaskListConfirmModal extends Modal {
	...
	onOpen() {
		...
		headerRow.createEl('th', { text: i18n.t('taskList.action'), cls: 'col-action' })
		headerRow.createEl('th', { text: i18n.t('taskList.localPath'), cls: 'col-path' })
		headerRow.createEl('th', { text: i18n.t('taskList.remotePath'), cls: 'col-path' })
		...
		this.tasks.forEach((task, index) => {
			...
			row.createEl('td', { text: getTaskName(task), cls: 'col-action' })
			row.createEl('td', { text: task.localPath, cls: 'col-path' })
			row.createEl('td', { text: task.remotePath, cls: 'col-path' })
		})
