export default {
	...
	settings: {
		...
		alipan: {
			...
			redirectUriWarning:
				'⚠️ 非 obsidian:// 协议的回调 URI 需要外部工具将授权码转发到 Obsidian。建议在阿里云盘开放平台将回调 URI 修改为 obsidian://alipan-sync/oauth',
			appIdRequired: '请先输入 App ID',
			advancedConfig: '高级配置',
			appSecret: 'App Secret (Client Secret)',
			appSecretDesc: '可选。桌面端应用通常不需要。',
			appSecretPlaceholder: '输入 App Secret（可选）',
			refreshToken: 'Refresh Token',
			refreshTokenDesc: '手动输入 Refresh Token',
			refreshTokenPlaceholder: '输入 Refresh Token',
			driveId: 'Drive ID',
			driveIdDesc: '云盘空间 ID',
			driveIdPlaceholder: '输入 Drive ID',
			saveAndConnect: '保存并连接',
			fieldsRequired: '请填写所有必需字段 (App ID, Refresh Token, Drive ID)',
			configSaved: '阿里云盘配置已保存',
			targetSpace: '目标空间',
			targetSpaceDesc: '选择文件同步到哪个空间。',
			targetSpaceHint: '「全部文件」= 资源盘，「备份文件」= 备份盘。',
			resourceDriveId: '资源盘 ID: {{id}}',
			backupDriveId: '备份盘 ID: {{id}}',
			resourceDrive: '全部文件（资源盘）',
			backupDrive: '备份文件（备份盘）',
			driveIdUnavailable: '所选空间的 Drive ID 不可用',
			driveSwitched: '已切换到{{space}}，下次同步将使用新空间',
		...
		cache: {
			...
			dumpName: '缓存管理',
			dumpDesc:
				'插件会在您的设备上保存远程文件夹的目录信息。当您更换设备时，这些信息会丢失，需要在首次同步前重新获取。如果您的文件数量较多，获取过程可能需要一些时间。导出功能可将这些信息保存到云端，导入功能则可将数据恢复到新设备，使您无需等待即可直接同步。',
			dump: '导出',
			restoreName: '导入缓存',
			restoreDesc:
				'从云端导入之前导出的缓存数据到当前设备。这样可以避免在新设备上等待漫长的文件扫描过程，让您直接进行同步。',
			restore: '导入',
			clearName: '清除本地缓存',
			clearDesc:
				'删除当前设备上的本地缓存数据。此操作无法撤销，会导致下次同步前需要重新建立缓存。仅影响当前设备，不会影响您在云端的数据。',
			clear: '清除',
			confirm: '确认清除',
			cleared: '缓存已成功清除',
			clearModal: {
	...
	sync: {
		...
		error: {
			...
			accountNotConfigured: '尚未配置云存储账号，请先在设置中配置账号信息',
		},
		...
		suggestUseClientForManyTasks:
			'提示：当同步任务较多时，同步可能需要较长时间。插件适用于桌面端和移动端的日常使用！',
		modalTitle: '同步进行中',
		cancelButton: '取消同步',
		progressText: '正在同步文件',
		fileOp: {
